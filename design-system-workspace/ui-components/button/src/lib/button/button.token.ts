...
import { BUTTON_CLICKED, BUTTON_LABEL, ButtonPComponent, ButtonWrapperComponent } from '@dso/ui-adapters-primeng';
...
@Component({
  standalone: true,
  selector: 'lib-button',
  imports: [CommonModule, ButtonPComponent],
  template: `<primeng-p-button [label]="label" (onclick)="clicked.emit()"></primeng-p-button>`
})
export class ButtonHostComponent {
  ...
  //       { provide: BUTTON_CLICKED, useValue: () => false }
  //     ]
  //   });
  // }
}
